...
func main() {
	...
	endTime := time.Now()
	lastServiceCheck := updateTimeState(endTime.Format("2006-01-02 15:04:05 -0700 MST"))

	if lastServiceCheck == "" {
		startTime = *aws.Time(endTime.UTC().Add(time.Second * -300))
		if opts.DebugMode {
			fmt.Println("No Last Service Check time available, defaulting to 5 minutes")
		}
	} else {
	...
	default:
		if opts.Statistics == "" || opts.NameSpace == "" || opts.DimensionName == "" {
			check.ExitUnknown("Generic Mode requires all of the following -s statistics, -n name space and -d dimension name")
		}
...
func addToPerf(resp *cloudwatch.GetMetricStatisticsOutput, stat string, nameSpace string) {
	...
	if resp.Datapoints == nil {
		...
		} else if nameSpace == "AWS/S3" || opts.Mode == "AWS/ELB.BackendConnectionErrors" {
...
func updateTimeState(newTime string) string {
	...
	path, err := retrievePath("AWS_Cloudwatch_", opts.Mode  + "," + opts.SearchName, paths)
	if err != nil {
		check.ExitUnknown("Error retrieving suitable file path")
	}
...
func retrievePath(filePrefix string, hashableData string, paths []string) (string, error) {
	// Create a file name using the requested prefix and a hash value
	// Tests the path locations to check where the file should be saved
	// Returns a string of the path and filename, along with error

	digest := make([]byte, len(hashableData))
	copy(digest[:], hashableData)
	hashableData = fmt.Sprintf("%x", md5.Sum(digest))

	fileName := filePrefix + "_" + string(hashableData[:]) + ".tmp"
	...
	var failedPaths string
 	...
 	for _, path := range paths {
		...
		if _, err := os.Stat(path); err == nil {
			...
			// List all paths that can't be used which will be shown to user if all paths are invalid
			failedPaths += path + " or "
		}
	}

	// Return error if none of the paths available are valid
	err := errors.New("Unable to use temporary file in path(s): " + failedPaths[:len(failedPaths)-4])
...
func checkPlugin() *plugin.Plugin {
	check := plugin.New("check_cloudwatch", "v1.1.0")
